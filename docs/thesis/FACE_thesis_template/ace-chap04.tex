\chapter[Document Ingestion and Understanding]{Document Ingestion and\\Understanding}
...
The implemented path proceeds from validation through technical PDF analysis,
extraction, selective optical character recognition (OCR), normalization,
...
enrichments. This chapter explains these stages and their dependencies.
...
Figure~\ref{fig:ingestion-pipeline} shows the complete flow from validation
and storage to one connected processing path, rather than a set of unrelated
document utilities.

\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.92\textwidth]{img/diagrams/DIA-03-ingestion-pipeline.pdf}
	\caption{Document ingestion and enrichment pipeline.}
...
extraction uses the Open XML SDK \cite{PdfPigDocs,MicrosoftOpenXMLSDK,ECMA376}.
...
analysis describes page representation, not subject matter; the distinction is
grounded in the PDF document representation standardized by ISO~32000-2
\cite{ISO32000PDF}.
...
The intended flow of these decisions is shown in
Figure~\ref{fig:pdf-classification-flow}.

\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.90\textwidth,height=0.82\textheight,keepaspectratio]{img/diagrams/DIA-04-pdf-page-classification.pdf}
	\caption{Deterministic page- and document-level technical PDF classification flow.}
	\label{fig:pdf-classification-flow}
\end{figure}
...
Rendering is provided by PDFtoImage using its PDFium runtime assets
\cite{PDFtoImageDocs}, and recognition is performed by Tesseract through the
TesseractOCR .NET wrapper \cite{Smith2007Tesseract,TesseractDocs}.
...
\begin{algorithm}[!htbp]
	...
	\begin{algorithmic}[1]
		...
		\ForAll{selected candidate pages}
			\State Render at up to 300 DPI within the 25,000,000-pixel limit
...
The wrapper package version and fallback label do not establish the exact
native-engine version used in the recorded evaluation.

Figure~\ref{fig:selective-ocr-flow} visualizes the page-level route and the
return to the unified extraction sequence.

\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.90\textwidth,height=0.78\textheight,keepaspectratio]{img/diagrams/DIA-05-selective-ocr-flow.pdf}
	\caption{Page-level selective OCR workflow.}
	\label{fig:selective-ocr-flow}
\end{figure}

\section{Document Understanding}
\label{sec:document-understanding}
\index{Document Understanding}
...
free-form prose \cite{OpenAIResponsesAPI}.
...
Passage-level retrieval provides the established setting for this engineering
trade-off \cite{Karpukhin2020DPR,Luan2021SparseDense}; overlap is the project's
deterministic mechanism for carrying limited context across boundaries.

The implementation measures tokens with the \texttt{cl100k\_base} encoding
through Microsoft.ML.Tokenizers. The package provides Tiktoken token counting
and conversion between text, token identifiers, and character indexes
\cite{MicrosoftMLTokenizers}. Character counts are retained for diagnostics,
but all size decisions are made
...
Figure~\ref{fig:chunk-overlap} shows three example chunks, their shared
suffixes, and the page/source metadata that accompanies each representation.

\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.93\textwidth]{img/diagrams/DIA-06-chunking-overlap.pdf}
	\caption{Token-aware chunking with bounded overlap and provenance.}
...
wrong-sized vectors. The provider documents both the selected embedding model
and the endpoint's configurable dimension parameter
\cite{OpenAIEmbeddingModel,OpenAIEmbeddingsAPI}; the value of 1,536 above is
the checked-in application configuration.
...
\texttt{vector(1536)} column \cite{PgvectorDocs}.
