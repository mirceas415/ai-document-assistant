...
Figure~\ref{fig:ui-grounded-answer} will show the final chat layout with a synthetic workspace and an answer containing multiple source markers. The screenshot is referenced here because it captures the interaction as a whole: navigation, history, question, answer, source cards, and composer. It must be captured with synthetic evaluation documents so that no private filename or unrelated conversation appears.

\thesisfigureplaceholder{UI-02}{Grounded answer with multiple citations in the active workspace.}{fig:ui-grounded-answer}
...
Figure~\ref{fig:ui-documents} is reserved for a workspace containing several synthetic documents in deliberately different states. The value of this image is not visual decoration; it demonstrates that ingestion is asynchronous and that one workspace may contain ready, processing, and failed items at the same time. The surrounding labels should remain legible in the final crop.

\thesisfigureplaceholder{UI-04}{Document management view showing processing states and document-level actions.}{fig:ui-documents}
...
Figure~\ref{fig:ui-ocr-diagnostics} focuses on a genuinely scanned synthetic PDF for which at least one page was routed to OCR. It should show both the aggregate OCR status and the page table, because either view alone would omit an important part of the selective design.

\thesisfigureplaceholder{UI-07}{Local OCR diagnostics for a synthetic scanned PDF selected by technical analysis.}{fig:ui-ocr-diagnostics}
...
Selecting an inline marker or its source card opens the source-details dialog reserved in Figure~\ref{fig:ui-source-details}. The dialog presents the source identifier, document name, chunk/page/heading location, and the stored excerpt. For a current source it may link the snapshot to a live document identifier; for a deleted document it can still show the historical snapshot and communicate that the original is no longer available. The interface does not pretend that the excerpt is a complete copy of the document.

\thesisfigureplaceholder{UI-03}{Source-details dialog showing validated document, page, chunk, heading, and excerpt provenance.}{fig:ui-source-details}
...
Figure~\ref{fig:ui-retrieval-details} should use an exact-identifier question against synthetic documents so that both lexical and metadata columns are meaningful. A screenshot where every result entered through the same channel would be technically valid but would poorly demonstrate the purpose of the panel.

\thesisfigureplaceholder{UI-10}{Advanced Retrieval Details showing vector, lexical, metadata, hybrid, reranked, and final positions.}{fig:ui-retrieval-details}
