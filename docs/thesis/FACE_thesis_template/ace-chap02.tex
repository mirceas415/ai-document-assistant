\chapter[Background and Fundamental Concepts]{Background and\\Fundamental Concepts}
...
\index{Retrieval-Augmented Generation (RAG)}
...
\index{Reciprocal Rank Fusion (RRF)}
...
where $n$ is the number of ranking channels, $w_i$ is the weight of channel $i$, $k$ is a positive rank constant, and $r_i(c)$ is the one-based rank of candidate $c$ in that channel. If the candidate is absent from a channel, that channel contributes zero.

The constant $k$ smooths the difference between adjacent ranks. Channel weights express an engineering preference but do not convert ranks into probabilities. RRF is an established information-retrieval technique \cite{Cormack2009RRF}. In this project, it is adapted through a weighted formulation that combines semantic, lexical, and bounded document-metadata contributions, together with deterministic deduplication and stable tie-breaking. Section~\ref{sec:hybrid-rrf} gives the actual formula and configuration.
...
Residual risk remains because model behavior is probabilistic and an apparently ordinary passage can carry adversarial text. The completed evidence therefore combines automated tests of malformed structured responses with application-level cases involving malicious synthetic document content, unsupported questions, and secret-disclosure requests. The project treats prompt injection as a security boundary to test and constrain, not as a problem that can be declared universally eliminated.
