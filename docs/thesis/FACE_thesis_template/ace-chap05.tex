...
These checks exclude a candidate whose vector is stale relative to its current text. The query requires current document-level aggregate metadata and validates each candidate's model, dimensions, timestamp, and content hash; it does not separately validate every sibling chunk before returning an eligible row. By contrast, the lexical channel in Section~\ref{sec:lexical-retrieval} can still retrieve ready text without current vectors. This difference is intentional and gives the hybrid design a useful degradation path.
...
\begin{table}[!htbp]
	...
	\small
	\begin{tabular}{@{}p{3.2cm}p{5.1cm}p{5.8cm}@{}}
		\toprule
		...
		Filename & Exact full-name or stem equality, plus term matches. & Identifies the source; a filename match does not establish factual support. \\
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.92\textwidth]{img/diagrams/DIA-07-hybrid-retrieval-pipeline.pdf}
	\caption{Hybrid retrieval and candidate-fusion pipeline.}
	\label{fig:hybrid-retrieval-pipeline}
\end{figure}
...
\begin{table}[!htbp]
	...
	\small
	\begin{tabular}{@{}p{5.1cm}p{2.5cm}p{6.5cm}@{}}
		\toprule
...
Figure~\ref{fig:rrf-fusion-concept} visualizes the same idea as contributions entering one rank-based score. It is deliberately a conceptual diagram rather than a graph of measured project quality.

\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.90\textwidth]{img/diagrams/DIA-08-rrf-fusion-concept.pdf}
	\caption{Conceptual weighted Reciprocal Rank Fusion of vector, lexical, and metadata rankings.}
	\label{fig:rrf-fusion-concept}
\end{figure}
...
\begin{table}[!htbp]
	...
	\small
	\begin{tabular}{@{}p{1.1cm}p{3.0cm}p{3.0cm}p{3.6cm}p{2.6cm}@{}}
		\toprule
...
\begin{table}[!htbp]
	...
	\small
	\begin{tabular}{@{}p{5.0cm}p{2.8cm}p{6.3cm}@{}}
		\toprule
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.75\textwidth,height=0.80\textheight,keepaspectratio]{img/diagrams/DIA-09-reranking-pipeline.pdf}
	\caption{Model-based reranking with fail-open behavior.}
	\label{fig:reranking-fallback}
\end{figure}

The design has an explicit latency and cost trade-off. Candidate generation requires one query embedding and database retrieval; reranking adds a second model request before answer generation. Larger candidate sets may improve the chance of presenting useful passages to the reranker, but consume more tokens and response time. The selected bounds limit request size and waiting time; actual latency and provider cost were not measured. Chapter~\ref{chap:testing-evaluation} reports the measured comparison and its limits.

\section{RAG Context Construction}
\label{sec:rag-context}

The final retrieved chunks cannot be copied into an unbounded prompt. Context construction turns the ordered Top-K result into a compact evidence package whose size, labels, and provenance are controlled by the backend. This is the retrieval-to-generation boundary of the RAG process \cite{Lewis2020RAG}.
...
\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.70\textwidth,height=0.82\textheight,keepaspectratio]{img/diagrams/DIA-10-grounded-citation-flow.pdf}
	\caption{Backend-controlled source and citation flow.}
	\label{fig:grounded-citation-flow}
\end{figure}
...
\begin{table}[!htbp]
	...
	\small
	\begin{tabular}{@{}p{2.9cm}p{5.5cm}p{5.7cm}@{}}
		\toprule
...
\begin{table}[!htbp]
	...
	\small
	\begin{tabular}{@{}p{3.6cm}p{5.9cm}p{4.6cm}@{}}
		\toprule
...
The security claim is therefore deliberately limited. The system establishes ownership boundaries, bounded requests, structured validation, fail-open reranking, evidence-only instructions, and authoritative citation membership. It does not claim to have solved prompt injection or to guarantee factual entailment. Chapter~\ref{chap:testing-evaluation} reports the no-evidence, citation, and prompt-injection cases measured in the final evaluation corpus. These limitations define the scope of the security claims: the contribution lies in combining practical controls at several layers and testing their deterministic properties, rather than treating probabilistic model behaviour as formally secure.
