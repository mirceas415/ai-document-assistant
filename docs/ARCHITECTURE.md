...
`IPdfPageRenderer` isolates rendering from ingestion and is implemented by `PdfiumPdfPageRenderer` using PDFtoImage/PDFium and SkiaSharp. `IOcrService` isolates recognition behind one result contract. Windows uses `TesseractOcrService` with the TesseractOCR managed/native wrapper and reuses one engine inside its scoped, serialized document-processing lifetime. macOS/Unix uses `TesseractCliOcrService` with the locally installed `tesseract` executable because the wrapper package ships Windows-native binaries; it streams the in-memory PNG to standard input and parses UTF-8 TSV from standard output without writing rendered pages to disk. The processor renders and recognizes one selected page at a time and disposes bitmaps, streams, OCR pages/processes, and pixel buffers promptly.
